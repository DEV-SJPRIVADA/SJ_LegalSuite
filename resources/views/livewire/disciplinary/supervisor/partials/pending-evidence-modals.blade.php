...
@if ($showSignaturePadModal)
    ...
    <div class="fixed inset-0 z-[90] flex items-center justify-center p-3 sm:p-4"
        ...
        x-init="init()"
        x-on:keydown.escape.window="$wire.closeWorkerSignaturePad()"
        role="dialog"
        aria-modal="true"
        aria-labelledby="worker-signature-title"
        wire:key="worker-signature-pad-{{ $signaturePadTarget }}">
        <div class="absolute inset-0 bg-black/60" wire:click="closeWorkerSignaturePad" aria-hidden="true"></div>
        <div class="relative w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-slate-200 dark:bg-dash-ink dark:ring-white/15">
            <div class="border-b border-slate-200 px-4 py-3 dark:border-white/10">
                <h2 id="worker-signature-title" class="text-base font-bold text-slate-900 dark:text-white">{{ $signaturePadTitle }}</h2>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Dibuje con el dedo o stylus en el recuadro.</p>
            </div>
            <div class="bg-slate-50 p-4 dark:bg-black/20">
                <canvas x-ref="canvas" class="h-52 w-full touch-none rounded-lg border-2 border-dashed border-slate-300 bg-white dark:border-white/20 dark:bg-white"
                    x-on:pointerdown="start($event)"
                    x-on:pointermove="draw($event)"
                    x-on:pointerup="end()"
                    x-on:pointerleave="end()"
                    x-on:pointercancel="end()"
                    x-on:touchstart.prevent="start($event)"
                    x-on:touchmove.prevent="draw($event)"
                    x-on:touchend.prevent="end()"></canvas>
            </div>
            <div class="flex flex-wrap justify-end gap-2 border-t border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-dash-ink/80">
                <button type="button" x-on:click="clear()"
                    class="inline-flex items-center rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-white dark:border-white/15 dark:text-white">
                    Limpiar
                </button>
                <button type="button" wire:click="closeWorkerSignaturePad"
                    class="inline-flex items-center rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-white dark:border-white/15 dark:text-white">
                    Cancelar
                </button>
                <button type="button"
                    x-on:click="(() => { const uri = exportDataUri(); if (!uri) { alert('Dibuje la firma antes de guardar.'); return; } $wire.saveCapturedSignature(uri); })()"
