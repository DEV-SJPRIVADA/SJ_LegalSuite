@php
    ...
    $wTotal = (int) ($workflowDonuts['total'] ?? 0);
    $stagePalette = [
...
<div class="disciplinary-dashboard mx-auto flex h-[calc(100dvh-3.25rem)] max-h-[calc(100dvh-3.25rem)] w-full max-w-[1600px] flex-col overflow-hidden px-3 py-2 sm:px-5 sm:py-3 lg:px-6"
    ...
    <header class="mb-2 flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-2 dark:border-white/10">
        <div class="min-w-0">
            ...
            <p class="mt-0.5 text-xs text-slate-500 dark:text-dash-muted">
                @if ($assignedOnly)
                    <span class="font-semibold tabular-nums text-slate-700 dark:text-slate-200">{{ number_format((int) ($kpis['total'] ?? 0)) }}</span> casos asignados
                    · <span class="font-semibold tabular-nums text-cyan-600 dark:text-cyan-300">{{ number_format((int) ($kpis['en_proceso'] ?? 0)) }}</span> en proceso
                @else
                    <span class="font-semibold tabular-nums text-slate-700 dark:text-slate-200">{{ number_format((int) ($kpis['total'] ?? 0)) }}</span> casos
                    · <span class="font-semibold tabular-nums text-cyan-600 dark:text-cyan-300">{{ number_format((int) ($kpis['en_proceso'] ?? 0)) }}</span> en proceso
                    · <span class="font-semibold tabular-nums text-amber-600 dark:text-amber-300">{{ number_format((int) ($kpis['pendientes'] ?? 0)) }}</span> pendientes
                @endif
            </p>
        </div>
        <x-dashboard.button href="{{ route('disciplinary.cases.index') }}" variant="ghost" class="shrink-0">
            {{ $assignedOnly ? 'Ver mis casos →' : 'Ver listado de casos →' }}
        </x-dashboard.button>
    </header>

    @if ($wTotal === 0)
        <div class="flex flex-1 items-center justify-center rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:text-dash-muted dark:ring-white/5">
            {{ $assignedOnly ? 'Aún no tiene casos asignados en su cartera.' : 'Aún no hay casos que coincidan con su alcance.' }}
        </div>
    @else
        <div class="flex min-h-0 flex-1 flex-col gap-2">
            {{-- Etapas A–F --}}
            <section
                class="shrink-0 overflow-visible rounded-xl border border-slate-200 bg-white px-2 pt-1.5 pb-0 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:shadow-dash-card dark:ring-white/5 sm:px-3 sm:pt-2"
                aria-label="Distribución de casos por etapa del flujo"
                wire:ignore
                x-ref="workflowDonuts"
            >
                <div class="grid w-full min-w-0 grid-cols-2 content-start items-start gap-2 sm:grid-cols-4 lg:grid-cols-7 lg:gap-1.5">
                    <div class="flex min-w-0 flex-col">
                        ...
                        <div data-workflow-donut="total" data-apex-chart-root class="h-[148px] w-full min-w-0 lg:h-[168px]"></div>
                    </div>

                    @foreach ($workflowDonuts['stages'] as $st)
                        @php $pal = $stagePalette[$st['letter']]; @endphp
                        <div class="flex min-w-0 flex-col" wire:key="workflow-donut-{{ $st['letter'] }}">
                            <p class="mb-0.5 shrink-0 px-0.5 text-center leading-snug" title="{{ $st['title'] }} (etapa {{ $st['letter'] }})">
                            ...
                            <div data-workflow-donut="{{ $st['letter'] }}" data-apex-chart-root class="h-[148px] w-full min-w-0 lg:h-[168px]"></div>
                        </div>
                    @endforeach
                </div>
            </section>
            ...
            {{-- Mapa + panel derecho --}}
            <div class="grid min-h-0 flex-1 grid-cols-1 items-stretch gap-2 lg:grid-cols-12 lg:gap-3">
                <section class="flex h-full min-h-[280px] min-h-0 flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:shadow-dash-card dark:ring-white/5 lg:col-span-7">
                    <div class="mb-1.5 flex items-center justify-between gap-2">
                        <div>
                            <h2 class="text-[10px] font-bold uppercase tracking-[0.14em] text-dash-muted">Casos por ciudad</h2>
                            <p class="mt-0.5 text-[10px] text-slate-500 dark:text-dash-muted">Mapa Colombia · pins por municipio DIVIPOLA</p>
                        </div>
                    ...
                    <div wire:ignore class="relative min-h-0 flex-1 min-h-[280px]">
                        <div id="disciplinary-colombia-map"
                             class="h-full min-h-[280px] w-full rounded-xl border border-slate-200/80 bg-slate-950/40 ring-1 ring-cyan-500/15 dark:border-white/10 dark:bg-black/30 dark:ring-fuchsia-500/20 z-0"
                        ...
                        @if (count($caseMapPins) === 0)
                            <div class="pointer-events-none absolute inset-0 flex items-center justify-center rounded-xl bg-slate-950/50 px-4 text-center backdrop-blur-[1px] dark:bg-black/45">
                                <p class="max-w-xs text-[11px] leading-relaxed text-slate-200">
                                    {{ $assignedOnly ? 'Sus casos asignados aún no tienen municipio DIVIPOLA.' : 'Sin casos georreferenciados en su alcance.' }}
                                </p>
                            </div>
                        @endif
                    </div>
                ...
                <aside class="grid h-full min-h-[280px] min-h-0 grid-rows-[minmax(0,0.85fr)_minmax(0,1.3fr)_minmax(0,0.85fr)] gap-2 lg:col-span-5">
                    <section class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:ring-white/5">
                        ...
                        <div class="min-h-0 flex-1 overflow-y-auto">
                            @if ($topMunicipalities === [])
                                <p class="flex h-full items-center justify-center text-center text-[11px] text-slate-500 dark:text-dash-muted">Sin datos con coordenadas.</p>
                            @else
                    ...
                    <section class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:ring-white/5">
                        <div class="mb-1.5 flex shrink-0 items-center justify-between gap-2">
                            ...
                            <span class="text-[10px] tabular-nums text-orange-400/90">{{ number_format($faultCasesTotal) }} caso(s)</span>
                        </div>
                        <div class="min-h-0 flex-1 overflow-y-auto pr-0.5">
                            @if ($byFault === [])
                                <p class="flex h-full items-center justify-center text-center text-[11px] text-slate-500 dark:text-dash-muted">Sin tipos de falta en catálogo.</p>
                            @else
                                <ul class="space-y-1">
                                    @foreach ($byFault as $fault)
                                        ...
                                        <li class="flex items-center gap-2" title="{{ $fault['name'] }}">
                                            ...
                                            <div class="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-white/10 ring-1 ring-white/5">
                                                <span
                                                    class="block h-full rounded-full transition-all {{ $total > 0 ? 'bg-gradient-to-r from-orange-400 to-orange-700 shadow-[0_0_8px_rgba(251,146,60,0.35)]' : 'bg-slate-600/25' }}"
                                                    style="width: {{ $barPct }}%"
                                                ></span>
                                            </div>
                    ...
                    @if ($assignedOnly && $myWorkload)
                        <section class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:ring-white/5">
                            <h2 class="mb-2 shrink-0 text-[10px] font-bold uppercase tracking-[0.14em] text-dash-muted">Mi carga</h2>
                            <div class="grid min-h-0 flex-1 grid-cols-2 gap-2">
                                <div class="flex flex-col justify-center rounded-lg bg-white/[0.03] px-2.5 py-2 ring-1 ring-white/5">
                                    <p class="text-[9px] font-bold uppercase tracking-wide text-dash-muted">Total asignados</p>
                                    <p class="text-xl font-bold tabular-nums text-slate-800 dark:text-white">{{ number_format($myWorkload['total']) }}</p>
                                </div>
                                <div class="flex flex-col justify-center rounded-lg bg-white/[0.03] px-2.5 py-2 ring-1 ring-white/5">
                                    <p class="text-[9px] font-bold uppercase tracking-wide text-dash-muted">Pendientes</p>
                                    <p class="text-xl font-bold tabular-nums text-amber-400">{{ number_format($myWorkload['pendientes']) }}</p>
                                </div>
                                <div class="flex flex-col justify-center rounded-lg bg-white/[0.03] px-2.5 py-2 ring-1 ring-white/5">
                                    <p class="text-[9px] font-bold uppercase tracking-wide text-dash-muted">En proceso</p>
                                    <p class="text-xl font-bold tabular-nums text-cyan-400">{{ number_format($myWorkload['en_proceso']) }}</p>
                                </div>
                                <div class="flex flex-col justify-center rounded-lg bg-white/[0.03] px-2.5 py-2 ring-1 ring-white/5">
                                    <p class="text-[9px] font-bold uppercase tracking-wide text-dash-muted">Finalizados</p>
                                    <p class="text-xl font-bold tabular-nums text-emerald-400">{{ number_format($myWorkload['finalizados']) }}</p>
                                </div>
                            </div>
                        </section>
                    @elseif (! $assignedOnly && $lawyerWorkloadTop !== [])
                        <section class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:border-white/10 dark:bg-white/[0.04] dark:ring-white/5">
                            <h2 class="mb-2 shrink-0 text-[10px] font-bold uppercase tracking-[0.14em] text-dash-muted">Carga por abogado</h2>
                            <ul class="min-h-0 flex-1 space-y-2 overflow-y-auto pr-0.5">
                                @foreach ($lawyerWorkloadTop as $row)
                                    ...
                                    <li>
                                        <div class="mb-0.5 flex items-center justify-between gap-2 text-[11px]">
                                            ...
                                            <span class="shrink-0 tabular-nums font-semibold text-fuchsia-300">{{ $row['total'] }}</span>
                                        </div>
                    ...
                    @else
                        <section class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-dashed border-slate-200 bg-slate-50/30 p-3 dark:border-white/10 dark:bg-white/[0.02]">
                            <p class="flex h-full items-center justify-center text-center text-[11px] text-slate-500 dark:text-dash-muted">Sin datos de carga.</p>
                        </section>
                    @endif
                </aside>
            </div>
