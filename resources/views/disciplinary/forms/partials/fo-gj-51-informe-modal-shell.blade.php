...
<div
    ...
    <div
        class="relative flex h-[calc(100dvh-1.5rem)] max-h-[calc(100dvh-1.5rem)] w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-slate-50 shadow-2xl ring-1 ring-slate-200 sm:h-[calc(100dvh-2rem)] sm:max-h-[calc(100dvh-2rem)] dark:bg-dash-ink dark:ring-white/15">
        <div class="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-dash-ink sm:px-5 sm:py-4">
            <div class="min-w-0">
                <p class="text-xs uppercase tracking-widest text-slate-500 font-semibold dark:text-dash-muted">Disciplinarios · FO-GJ-51</p>
                <h2 id="fo51-main-modal-title" class="mt-1 text-lg font-bold text-slate-900 dark:text-white">Informe disciplinario</h2>
            </div>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto overflow-x-auto overscroll-y-contain">
            <div class="px-3 py-4 sm:px-6 sm:py-6">
                @error('fo51_action')
                    <div class="mb-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-800 ring-1 ring-red-200 dark:bg-red-500/15 dark:text-red-200 dark:ring-red-500/30">
                        {{ $message }}
                    </div>
                @enderror
