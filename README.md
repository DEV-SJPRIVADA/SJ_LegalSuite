...
**Márgenes cartas FO-GJ:** Dompdf no respeta `box-sizing:border-box` (`width:100%` + padding = margen izquierdo y borde derecho cortado). Modelo general: `@page { margin: 0 }` + `.ogj-page { width: 7.5in; margin: 0.5in; padding: 0 }` (Letter 8.5in). **FO-GJ-03** es flujo continuo (sin planificador de secciones): `@page { margin: 0.5in }`, letterhead `position: fixed` (Dompdf lo repite por página física; evitar thead de una sola celda alta), cuerpo con `padding-top: 96px` y párrafos justificados, “Página N de M” vía canvas tras render (`PAGE_NUM`/`PAGE_COUNT`); la forma canónica cabe en **1** Letter; campos largos (p. ej. cargos) pueden ocupar N páginas con el mismo encabezado. Encabezado **25% / 50% / 25%**. En blanco, numerales 66/68/76 son guías (no listas fijas).
