...
**Disciplinario — Etapa B (citación):** **B.1 Programación de diligencia:** `coordination_started_at` con **Iniciar coordinación**; planeación publica `proposed_slots`; abogado confirma fecha (`confirmCitationSlot`). **B.2 Notificación física:** tras fecha confirmada, abogado solicita información (`LAWYER_NOTIFICATION_REQUEST`); planeación responde con formulario estructurado (`NOTIFICATION_COORDINATION`, `notification_payload`) y asigna supervisor (rol `supervisor`). Operaciones puede **reasignar supervisor** (`SUPERVISOR_NOTIFICADOR_REASIGNADO`). FO-GJ-03 exige fecha confirmada + información de notificación + supervisor (`DisciplinaryCitationNotificationService`). Servicio: `FoGj03CitationService`; migración `2026_06_03_100000_add_citation_notification_fields.php`.

**Disciplinario — coordinaciones con planeación:** bandeja `Livewire\Disciplinary\Coordinations\Index` para rol `planeacion`; responde **programación de diligencia** (`proposed_slots`) y, si hay solicitud pendiente, el formulario **B.2 notificación física** (fecha ingreso, turno, zona, supervisor). Cierre con `coordination_status` / `closed_at` (`2026_06_02_080000_add_closure_fields_to_disciplinary_agenda_threads.php`); actuación `COORDINACION_CERRADA`. Políticas: `postAgendaPlanning`, `postNotificationCoordination`.
...
| `GET /disciplinary/coordinations` | Bandeja planeación: diligencia (slots) + notificación física (formulario B.2) |
