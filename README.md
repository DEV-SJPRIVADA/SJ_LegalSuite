...
**Disciplinario — Etapa C (Fases 1–3 implementadas):** al abrir la gestión el día de la diligencia, el **primer paso** es registrar **Asistió** o **No asistió** (`diligence_attendance`, decisión irreversible). **Si asistió:** FO-GJ-04 + firma del trabajador + bloqueo de avance a decisión sin acta (`DisciplinaryDiligenceWorkflowService`). **Si no asistió:** **FO-GJ-44** (`FoGj44DraftService`, `FoGj44ConstanciaService`, `DocumentType::CONSTANCIA_INASISTENCIA`) → transición automática a **JUSTIFICACION_PENDIENTE** (2 días) → **FO-GJ-54** + `acceptJustification` o `rejectJustification` → **Comité disciplinario** (`COMITE_DISCIPLINARIO`). Rutas PDF: `fo-gj-44/pdf`, `fo-gj-54/pdf`, `comite-acta/pdf`. **Fase 4 (futuro):** FO-GJ-55 + acta de ampliación.

**Disciplinario — Comité disciplinario (acta):** estado `COMITE_DISCIPLINARIO` tras rechazo o vencimiento de justificación. En detalle del caso: panel **Etapa C · Comité** (stepper 5 pasos con paso «Acta de comité»); pila de etapas **solo C + A** (sin Etapa B). Barra de acciones: **Diligenciar comité** → **Vista previa PDF** → **Generar y guardar** (+ **Consultar FO-GJ-44** opcional). Modal: decisión/acuerdo, integrantes (nombre, cargo, firma capturable). Servicios: `ComiteDraftService`, `ComiteActaService` → HTML `comite-acta-filled-download` + **Browsershot** (`DocumentType::ACTA_COMITE`). Con **membrete** cargado en Formatos: imagen PNG/JPEG a sangre (8.5×11″, `@page margin: 0`, `HtmlLetterPdfGenerator` con márgenes cero), cuerpo tipo carta oficial (fecha en Santiago de Cali, ACTA/CASO/ASUNTO, relato) y firmas en **2 columnas** (Firma / Nombre / Cargo). Sin membrete: encabezado estándar `official-letter-pdf-shell`. Migración `2026_06_18_100000_diligence_justification_and_comite` (`comite_payload`, `comite_generated_at`, etc.). Políticas: `editComiteDraft`, `previewComite`, `generateComite`. Tests: `CaseDetailStageViewsTest` (comité), `OrganizationLetterheadTest`.
...
**Disciplinario — Captura de firma (móvil y mesa digitalizadora):** lienzo **`worker-signature-pad.js`** con franja horizontal centrada (ancho `calc(100vw - margen)`, altura fija **17.5rem**). En móvil: dedo (`pointer`/`touch`). En PC: lápiz Wacom vía Pointer Events (mapear la mesa a **un solo monitor** y maximizar el navegador). **Livewire:** `<x-disciplinary.signature-capture-modal>` en FO-GJ-03 (supervisor: trabajador y testigos) y FO-GJ-04 (trabajador en acta). **Formularios POST (Alpine):** `<x-disciplinary.signature-capture-modal-alpine>` + `sjFo51PreparerSignature()` en **FO-GJ-51** (firma de quien elabora el informe). Validación backend: regla `PngSignatureDataUri`.
