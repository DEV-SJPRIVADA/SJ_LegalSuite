...
**Márgenes cartas FO-GJ:** Dompdf no respeta `box-sizing:border-box` (`width:100%` + padding = margen izquierdo y borde derecho cortado). Modelo: `@page { margin: 0 }` + `.ogj-page { width: 7.5in; margin: 0.5in; padding: 0 }` (Letter 8.5in). **FO-GJ-03** usa páginas HTML explícitas (`FoGj03DocumentPaginator` + `ogj-page-break`) con encabezado en cada hoja; meta **4 filas** (~76px) y “Página N de M” en el HTML; el empaque evita huecos inferiores en p.1 (backfill de evidencia/artículos). Párrafos justificados; forma canónica en **1** Letter y campos largos en N páginas. Encabezado **25% / 50% / 25%**. En blanco, numerales 66/68/76 son guías.
...
- [x] FO-GJ-03 PDF multipágina estable: páginas Letter explícitas (`FoGj03DocumentPaginator`), encabezado HTML en cada hoja, sin `position:fixed` Dompdf; empaque densificado + backfill para reducir hueco en p.1
