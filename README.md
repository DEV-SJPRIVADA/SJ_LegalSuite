...
**Disciplinario — FO-GJ-DECISION (comunicado genérico):** plantilla `decision-comunicado-body` para ramas distintas de 46/47 (p. ej. terminación hasta FO-GJ-45). Diligenciamiento: asunto, narrativa, relevo. Notificación firmada: `decision-comunicado-signed-notification-download`.

**Disciplinario — Captura de firma (móvil y mesa digitalizadora):** lienzo **`worker-signature-pad.js`** con franja horizontal centrada (ancho `calc(100vw - margen)`, altura fija **17.5rem**). En móvil: dedo (`pointer`/`touch`). En PC: lápiz Wacom vía Pointer Events (mapear la mesa a **un solo monitor** y maximizar el navegador). **Livewire:** `<x-disciplinary.signature-capture-modal>` en FO-GJ-03, FO-GJ-46, FO-GJ-47 y FO-GJ-DECISION (supervisor: trabajador y testigos) y FO-GJ-04 (trabajador en acta). **Formularios POST (Alpine):** `<x-disciplinary.signature-capture-modal-alpine>` + `sjFo51PreparerSignature()` en **FO-GJ-51** (firma de quien elabora el informe). Validación backend: regla `PngSignatureDataUri`.
...
| **FO-GJ-DECISION** | Comunicado de decisión genérico | `decision-comunicado-blank-download` + `decision-comunicado-filled-download` (terminación/cierres hasta FO-GJ-45) |
...
**Supervisor (`supervisor`):** sin `view` / `viewAny` ni dashboard disciplinario. Carga evidencia **FO-GJ-03** si figura como **`notification_supervisor_user_id`**, y evidencia de decisión (**FO-GJ-46** / **FO-GJ-47** / FO-GJ-DECISION) si figura como **`decision_notification_supervisor_user_id`**, desde **`GET /disciplinary/evidences-pending`**. Políticas `viewFoGj03NotificationForSupervisor` y `viewDecisionComunicadoForSupervisor` habilitan los modales HTML carta (sin abrir el expediente). Front: `worker-signature-pad.js` (lienzo táctil); previsualización PDF temporal vía Livewire (`config/livewire.php` → `preview_mimes` incluye `pdf`).
...
| `GET /disciplinary/evidences-pending` | Cola supervisor (`PendingEvidenceIndex`): citación FO-GJ-03 y decisión FO-GJ-46/47/DECISION — **Cargar evidencia PDF** (escaneado con vista previa inline) o **Notificación** (HTML carta + firma/rechazo con testigos → PDF Browsershot). Sin acceso al expediente. |
...
| `GET /disciplinary/cases/{case}/decision-comunicado/pdf` | Vista previa comunicado de decisión FO-GJ-46 / FO-GJ-47 / FO-GJ-DECISION (`previewDecisionComunicado`). |
| `POST /disciplinary/cases/{case}/decision-comunicado/generate` | Genera y almacena FO-GJ-46, FO-GJ-47 o FO-GJ-DECISION (`generateDecisionComunicado`; borrador completo y notificación coordinada). |
...
- [x] Etapa D: FO-GJ-46 (llamado de atención) y FO-GJ-47 (suspensión) + coordinación (inicio suspensión calculada) + evidencia supervisor
- [ ] Exportación PDF de actuaciones con plantillas FO-GJ restantes desde el caso (FO-GJ-45 acta de archivo)
