...
- **Etapa C (diligencia y acta):** **FO-GJ-04** reemplaza al código histórico FO-GJ-42; acta HTML→PDF con textos legales fijos y **paginación híbrida** (`FoGj04PagePlanner`); fecha del incumplimiento y cargos heredados del **FO-GJ-03**; modal con cuestionario dinámico (pregunta + respuesta obligatoria) y manifestación **SI QUIERO RESPONDER** / **NO DESEA RESPONDER**; vista previa y generación al expediente (`FoGj04DraftService`, `FoGj04DiligenceActaService`). **Pendiente de evolución:** registro **Asistió / No asistió** al inicio de Etapa C, FO-GJ-44 diligenciable desde expediente, bloqueo de avance a decisión sin acta y firma del trabajador en el PDF (ver plan en roadmap).
...
**Disciplinario — Etapa C (plan de evolución acordado):** al abrir la gestión el día de la diligencia, el **primer paso** será registrar **Asistió** o **No asistió** (decisión persistida en el expediente). **Si asistió:** flujo actual (diligenciar FO-GJ-04, vista previa, generar y guardar) + **bloquear** «Siguiente etapa → Decisión» sin acta generada + **firma del trabajador** en el PDF. **Si no asistió:** flujo de **FO-GJ-44** (constancia de inasistencia) diligenciable y generable desde el expediente; luego ventana de **2 días calendario** para justificación — aceptada → **FO-GJ-54** y reprogramación; rechazada o sin justificación → **Comité disciplinario**. **Ampliación (futuro):** **FO-GJ-55** (citación ampliación) + acta de ampliación y reingreso a diligencia. Hoy el avance a decisión **no exige** FO-GJ-04 generado; FO-GJ-44/54 solo plantilla en blanco en Formatos; la inasistencia tiene métodos en `DisciplinaryWorkflowService` pero **sin UI** en expediente.
...
- [ ] **Fase 1:** Registro **Asistió / No asistió** al inicio de Etapa C (primer paso del abogado el día de la diligencia)
- [ ] **Fase 2 (rama asistió):** Bloquear avance a decisión sin FO-GJ-04 generado + firma del trabajador en acta
- [ ] **Fase 3 (rama no asistió):** FO-GJ-44 desde expediente + justificación (2 días) + FO-GJ-54 o Comité disciplinario
...
- [x] Etapa C en detalle del caso (diligencia): FO-GJ-04 (reemplaza FO-GJ-42), plantilla oficial multipágina con paginación híbrida, escala tipográfica unificada en PDF, cargos desde FO-GJ-03, modal con cuestionario pregunta+respuesta y manifestación SI/NO, pila C→B→A, Etapa B solo lectura compacta, encabezado compacto y botones `<x-ui.btn>`
- [ ] Exportación PDF de actuaciones con plantillas FO-GJ restantes desde el caso
- [ ] Vista Kanban "Mi pipeline" por abogado
- [ ] Tests Pest ampliados (parcial: `DisciplinaryCitationNotificationTest`, `FoGj03DraftTest`, `FoGj04DraftTest`, `DisciplinaryLawyerPoolClaimTest`, `CaseDetailStageViewsTest`)
