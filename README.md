...
**Disciplinario — evidencia de decisión:** `canReceiveDecisionEvidence()` exige comunicado generado (`decision_comunicado_generated_at` o documento FO-GJ-DECISION en expediente). Carga vía `uploadDecisionEvidence` (PDF) o `uploadSignedDecisionNotification` (firma en pantalla). Matriz en `canUserUploadDecisionEvidence()`: titular, revisor del informe, `disciplinary.review-inform-all`, `decision_notification_supervisor_user_id`, dirección jurídica. Modal FO-GJ-DECISION con `viewDecisionComunicadoForSupervisor`. Tests: `DecisionStageCompletionTest.php`.
