...
**Disciplinario — Captura de firma (móvil y mesa digitalizadora):** componente **`signature-capture-modal`** con franja horizontal centrada (ancho `calc(100vw - margen)`, altura fija **17.5rem**). En móvil: dedo (`pointer`/`touch`). En PC: lápiz Wacom vía Pointer Events (mapear la mesa a **un solo monitor** y maximizar el navegador para mejor área útil). Usado en FO-GJ-03 (supervisor: trabajador y testigos) y FO-GJ-04 (trabajador en acta).
...
- [x] Captura de firma unificada (`signature-capture-modal` + `worker-signature-pad.js`: móvil táctil, Wacom en PC)
...
- [ ] Tests Pest ampliados (parcial: `DisciplinaryCitationNotificationTest`, `FoGj03DraftTest`, `FoGj04DraftTest`, `DiligenceAttendanceTest`, `DisciplinaryLawyerPoolClaimTest`, `CaseDetailStageViewsTest`)
