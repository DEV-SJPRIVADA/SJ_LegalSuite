...
**Márgenes cartas FO-GJ:** Dompdf no respeta `box-sizing:border-box` (`width:100%` + padding = margen izquierdo y borde derecho cortado). Modelo: `@page { margin: 0 }` + `.ogj-page { width: 7.5in; margin: 0.5in; padding: 0 }` (Letter 8.5in). **FO-GJ-03** pagina con `FoGj03PagePlanner`: `evidence` (traslado) **nunca** comparte hoja con `opening` (evita overflow Dompdf sin encabezado); encabezado **25% / 50% / 25%**; test Dompdf comprueba páginas físicas = “N de M”.
