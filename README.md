...
- **Etapas A y B (informe + citación):** revisor de operaciones obligatorio al enviar FO-GJ-51; coordinación explícita con planeación en citación; selección visual de fecha definitiva; **coordinación de notificación física (B.2)** con asignación de supervisor por planeación; avance a diligencia desde la UI con checklist de requisitos (`docs/GAP_DISCIPLINARIO_ETAPAS_A_B.md`). **Evidencia de citación (PDF):** habilitada solo tras generar FO-GJ-03; pueden cargarla el abogado titular, el **supervisor asignado para notificación** (`notification_supervisor_user_id`), el usuario de operaciones que autorizó el FO-GJ-51 (`reviewed_by`), dirección de operaciones (`disciplinary.review-inform-all`) y dirección jurídica (`admin` / `disciplinary.assign`). El supervisor la carga desde **Evidencias pendientes** sin acceso al expediente.
...
| **Evidencias pendientes** (supervisor) | Cola mínima (`PendingEvidenceIndex`) filtrada por `notification_supervisor_user_id`: N° caso, trabajador, estado **Evidencia de citación pendiente**, fecha FO-GJ-03 y **Cargar evidencia**. Solo casos con FO-GJ-03 generado y sin evidencia; no expone detalle del expediente. |
...
**Disciplinario — Etapa B (citación):** chat libre abogado ↔ planeación (`AgendaMessageKind::GENERAL`); planeación publica fechas con **`proposed_slots`** (`PLANNING_RESPONSE`) sin duplicarlas en el texto del mensaje — solo el bloque **Fechas propuestas** en `agenda-message.blade.php`. Orden: (1) coordinación + chat, (2) modal **Proponer fechas de diligencia** en coordinaciones, (3) abogado confirma slot, (4) al publicar slots se habilita **Registrar notificación y supervisor** en coordinaciones (`canPlanningRegisterNotification`), (5) FO-GJ-03 y evidencia. Migración `2026_06_03_120000_reclassify_informal_agenda_messages_as_general.php` corrige mensajes de chat antiguos guardados como `lawyer_request`. Tests: `DisciplinaryCitationStageFlowTest.php`, `DisciplinaryPortalRouteTest.php`.

**Disciplinario — coordinaciones con planeación:** `Coordinations\Index`: chat (**Enviar mensaje**), botón **Proponer fechas de diligencia** (modal → `postPlanningMessage` con slots), **Registrar notificación y supervisor** (modal cuando `canPlanningRegisterNotification`, tras fechas de diligencia en el hilo). Badge **Fechas pendientes** cuando `awaitingPlanningDiligenceSlots()`. `data-live-case-id` + `wire:poll` para actualizar el hilo. Cierre definitivo del hilo al avanzar a **diligencia** (`confirmAdvanceFromCitacion`); el abogado **oculta/muestra** el chat en expediente sin cerrar el hilo. Políticas: `postAgendaPlanning`, `postNotificationCoordination`.

**Disciplinario — evidencia de citación:** `canReceiveCitationEvidence()` exige FO-GJ-03 generado y documento asociado. Carga vía `uploadCitationEvidence`. Matriz en `canUserUploadCitationEvidence()`: titular, `informeSubmission.reviewed_by`, `disciplinary.review-inform-all`, `notification_supervisor_user_id`, dirección jurídica; excluye planeación y supervisores no asignados. Cola `evidences-pending` sin `view`/`viewAny` de expediente. Tests: `DisciplinaryCitationNotificationTest.php`.
...
| `supervisor@sjlegalsuite.local` | supervisor | FO-GJ-51 + cola **Evidencias pendientes** (solo casos donde `notification_supervisor_user_id` coincide); sin listado ni detalle de expedientes |
...
Si actualizas código y una BD ya tenía migraciones viejas aplicadas, ejecuta **`php artisan migrate`** (p. ej. `2026_06_03_100000_add_citation_notification_fields` para Etapa B.2). En desarrollo suele bastar **`migrate:fresh --seed`**; en datos reales, no editar migraciones ya ejecutadas sin plan de alter explícito.
...
**Planeación (`planeacion`):** no tiene `view` / `viewAny` sobre expedientes. Opera en **`GET /disciplinary/coordinations`**: propone fechas de diligencia y completa la **coordinación de notificación física** (`postNotificationCoordination`). No puede `uploadCitationEvidence` ni `reassignNotificationSupervisor`. El abogado **oculta o muestra** el chat en el expediente; el hilo se **cierra al avanzar a diligencia** (`closeCoordination` automático).

**Supervisor (`supervisor`):** sin `view` / `viewAny` ni dashboard disciplinario. Carga evidencia FO-GJ-03 solo si figura como **`notification_supervisor_user_id`** del caso, desde **`GET /disciplinary/evidences-pending`** (no por `reporter_id` ni autor del informe).
...
| `GET /disciplinary/evidences-pending` | Cola para supervisor asignado a notificación: carga de evidencia de citación (sin acceso al expediente) |
| `GET /disciplinary/coordinations` | Bandeja planeación: chat, modal fechas de diligencia (`proposed_slots`) y modal notificación/supervisor |
...
| `GET /disciplinary/cases/{case}/fo-gj-03/pdf` | Vista previa / descarga FO-GJ-03 (requiere fecha de diligencia confirmada). |
| `POST /disciplinary/cases/{case}/fo-gj-03/generate` | Genera FO-GJ-03 si hay fecha confirmada, información B.2 completa y supervisor asignado (`DisciplinaryCitationNotificationService`). Notifica al supervisor, operaciones aprobador y dirección de operaciones. |
...
- [ ] Tests Pest ampliados (parcial: `DisciplinaryCitationNotificationTest`, `DisciplinaryLawyerPoolClaimTest`)
