...
| **Evidencias pendientes** (supervisor) | Cola mínima (`PendingEvidenceIndex`) filtrada por `notification_supervisor_user_id`: N° caso, trabajador, estado **Evidencia de citación pendiente**, fecha FO-GJ-03 y dos acciones por fila. **Cargar evidencia PDF** (fase A): selector oculto → modal de **vista previa** → tipo **Citación firmada** o **Rechazo con testigos** → **Confirmar y cargar**. **Notificación** (fase B): modal con hoja **carta** (`8.5in × 11in`, escala responsive) y HTML oficial FO-GJ-03; selector **Firmada / Rechazo con testigos**. Firmada: firma del trabajador en «Recibido por». Rechazada: texto **Se niega a firmar** en columna del guarda + dos bloques **Testigo** (firma, nombre, cédula) en el formato y en el pie del modal; **Cargar firmado** genera PDF con Browsershot (`CitationNotificationSigningService`) y sube evidencia según `CitationEvidenceType`. Política `viewFoGj03NotificationForSupervisor`. Tests: `PendingEvidenceUploadTest.php`. |
