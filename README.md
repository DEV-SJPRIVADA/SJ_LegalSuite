...
**Márgenes cartas FO-GJ:** Dompdf no respeta `box-sizing:border-box` (`width:100%` + padding = margen izquierdo y borde derecho cortado). Modelo: `@page { margin: 0 }` + `.ogj-page { width: 7.5in; margin: 0.5in; padding: 0 }` (Letter 8.5in), aplicado también a **FO-GJ-03** continuo. Su letterhead `position: fixed` se ancla a `top/left: 0.5in`; el cuerpo reserva `94px` para iniciar bajo la cabecera. El meta tiene **4 filas** de altura fija (~76px), de modo que “Página N de M” (canvas) quede dentro del marco. Párrafos justificados; forma canónica en **1** Letter y campos largos en N páginas. Encabezado **25% / 50% / 25%**. En blanco, numerales 66/68/76 son guías.
