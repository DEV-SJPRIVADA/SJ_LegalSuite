...
- **Etapa C (diligencia y acta):** **FO-GJ-04** reemplaza al código histórico FO-GJ-42; acta HTML→PDF de 2 páginas con textos legales fijos; fecha del incumplimiento y cargos heredados del **FO-GJ-03**; modal con cuestionario dinámico (vacío al inicio), manifestación **SI QUIERO RESPONDER** / **NO DESEA RESPONDER**, vista previa y generación al expediente (`FoGj04DraftService`, `FoGj04DiligenceActaService`); firma del trabajador en blanco (captura en pantalla pendiente).
...
**Disciplinario — FO-GJ-04 (acta de diligencia):** sustituye **FO-GJ-42** en catálogo y código (`OfficialFormsCatalog`, sin alias). Plantilla 2 páginas `fo-gj-04-body.blade.php` / `fo-gj-04-filled-download` con textos legales fijos (citación previa, términos 1–5, manifestación, cierre antes del cuestionario). **Fecha del incumplimiento** y **formulación de cargos** se leen del FO-GJ-03 diligenciado (`fo_gj_03_payload.breach_date`, `charges_description`); el modal no duplica esos campos. Diligenciamiento: `FoGj04DraftService` (`fo_gj_04_payload`, `fo_gj_04_draft_completed_at`); generación: `FoGj04DiligenceActaService` → `DocumentType::ACTA_DILIGENCIA` + `fo_gj_04_generated_at`. Modal Etapa C: hora inicio/fin, manifestación **SI QUIERO RESPONDER** / **NO DESEA RESPONDER**, cuestionario dinámico (vacío al abrir). Firma del abogado en PDF; hueco del trabajador en blanco (captura en pantalla pendiente). Ruta vista previa: `GET /disciplinary/cases/{case}/fo-gj-04/pdf` (`FoGj04CaseController`). Políticas: `editFoGj04Draft`, `previewFoGj04`, `generateFoGj04`. Migración: `2026_06_10_100000_fo_gj_04_diligence_acta_draft`. Tests: `FoGj04DraftTest.php`.
...
- [x] Etapa C en detalle del caso (diligencia): FO-GJ-04 (reemplaza FO-GJ-42), plantilla oficial 2 páginas, cargos desde FO-GJ-03, modal con cuestionario dinámico y manifestación SI/NO, pila C→B→A, Etapa B solo lectura compacta
