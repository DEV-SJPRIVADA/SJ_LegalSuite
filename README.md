...
**Disciplinario — Etapa B (citación):** chat libre abogado ↔ planeación (`AgendaMessageKind::GENERAL`); planeación publica fechas con **`proposed_slots`** (`PLANNING_RESPONSE`) sin duplicarlas en el texto del mensaje — solo el bloque **Fechas propuestas** en `agenda-message.blade.php`. Orden: (1) coordinación + chat, (2) modal **Proponer fechas de diligencia** en coordinaciones, (3) abogado confirma slot, (4) abogado solicita notificación, (5) modal **Registrar notificación y supervisor** (`completeNotificationInformation`), (6) FO-GJ-03 y evidencia. Migración `2026_06_03_120000_reclassify_informal_agenda_messages_as_general.php` corrige mensajes de chat antiguos guardados como `lawyer_request`. Tests: `DisciplinaryCitationStageFlowTest.php`, `DisciplinaryPortalRouteTest.php`.

**Disciplinario — coordinaciones con planeación:** `Coordinations\Index`: chat (**Enviar mensaje**), botón **Proponer fechas de diligencia** (modal → `postPlanningMessage` con slots), **Registrar notificación y supervisor** (modal si hay solicitud pendiente). Badge **Fechas pendientes** cuando `awaitingPlanningDiligenceSlots()`. `data-live-case-id` + `wire:poll` para actualizar el hilo. Cierre: `coordination_status` / `closed_at`; `CitationStageProgress::blockersBeforeClosingCoordination()`. Políticas: `postAgendaPlanning`, `postNotificationCoordination`.
